...
h20_gbm_tune <- function(path_data, status, type){
  ...
  hyper_params = list( max_depth = seq(1,16,2),
                       learn_rate = c(0.01, seq(0.05, 0.3, 0.05)))
  ...
  topLRate = sortedGrid@summary_table$learn_rate[1:5]
  minLRate = min(as.numeric(topLRate))
  maxLRate = max(as.numeric(topLRate))
  
  hyper_params = list(
    ## restrict the search to the range of max_depth established above
    max_depth = seq(minDepth,maxDepth,1),
    ## restrict the search to the range of max_depth established above
    learn_rate = seq(minLRate,maxLRate,0.05),
  ...
  cat(paste0("The deviance on the test data is ", h2o.residual_deviance(gbm, train = TRUE)))
