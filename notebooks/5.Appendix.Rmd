...
Though we present outputs for all three models below, we  show that the best model is Model 1, using `caret::resamples`. This function allows us to iteratvly build models using the training data and measure performance each round. In the end, we have resampled measures of each performance metric - rsquared, RSME and MSE. Before comparing model performance, however, we first train the models - this is done to help in determining a range of parmters that would fit the data best for `caret::resamples`. 
...
gbm_regressor_baiSpeciesAgeEP <-
  gbm(BAI_GR ~ .,
      data = 
        ...
        mutate(Species = factor(Species)),
      n.trees = 1000,
      interaction.depth = 4, #max depth 
      shrinkage = 0.3, #learning rate
      n.minobsinnode = 9, #col_sample_rate 
      bag.fraction =  0.41, # sample_rate,
      verbose = FALSE,
      n.cores = NULL,
      cv.folds = 5)
