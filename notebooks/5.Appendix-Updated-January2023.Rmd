...
## Train Models{.tabset}
Below, we show the best parameters for the models, given the data. 

### Model 1: Tree Age + Plant Traits + Environmental Conditions{.tabset}
...
```{r include=FALSE}
set.seed(1)
grid <- expand.grid(n.trees=c(100, 150, 200, 500, 1000), 
                    interaction.depth=c(3), 
                    shrinkage=c (0.05), 
                    n.minobsinnode=c(13))
gbmModel1 <- caret::train(x = 
                    rgr_msh_na %>% filter(Group == "Train")%>% 
                    filter(!is.na(BAI_GR))%>%
                    select(any_of(c(EnvironmentalVariablesKeep,PlantTraitsKeep,
                                    "Tree.Age", "BAI_GR","julian.date.2011"))) %>%
                      select(-BAI_GR),
                  y = 
                    rgr_msh_na %>% filter(Group == "Train")%>% 
                    filter(!is.na(BAI_GR)) %>% select(BAI_GR)%>% reduce(c), 
                  method = 'gbm', 
                  tuneGrid = grid, 
                  verbose = FALSE)
set.seed(1)
grid <- expand.grid(n.trees=c(100, 150, 200, 500, 1000), 
                    ...
                    shrinkage=c (0.05), 
                    n.minobsinnode=c(6))
gbmModel2 <- caret::train(x = 
                    rgr_msh_na %>% filter(Group == "Train")%>% 
                    filter(!is.na(BAI_GR))%>%
                    select(any_of(c(EnvironmentalVariablesKeep,
                                    "Species", "Tree.Age", "BAI_GR","julian.date.2011"))) %>%
                    mutate(Species = factor(Species)) %>% select(-BAI_GR),
                  y = 
                    rgr_msh_na %>% filter(Group == "Train")%>% 
                    filter(!is.na(BAI_GR)) %>% select(BAI_GR)%>% reduce(c), 
                  method = 'gbm', 
...
ModelPerformanceCompare <- caret::resamples(list(`Model 1 - Plant Traits + Environmental Conditions`=gbmModel1,
                                 `Model 2 - Species Identity + Environmental Conditions`=gbmModel2,
                                 `Model 3 - Species Identity + Environmental Conditions + Plant Traits` = gbmModel3))
...
```{r echo=FALSE}
summary(ModelPerformanceCompare)
```

### Boxplot

```{r echo=FALSE}
Result <- ModelPerformanceCompare$values
colnames(Result) <- str_replace(colnames(Result), "~", "_")
 Result %>%
  select(matches("RMSE"))%>%
  pivot_longer(cols = everything(),
               names_to = "Model", names_pattern = "(.*)_", values_to = "Value")%>% 
   mutate(Measure = "Root-Mean-Square Error") %>%
   bind_rows(Result %>%
  select(matches("MAE"))%>%
  pivot_longer(cols = everything(),
               names_to = "Model", names_pattern = "(.*)_", values_to = "Value")%>% 
   mutate(Measure = "Mean Absolute Error"))%>%
   bind_rows(Result %>%
  select(matches("Rsquared"))%>%
  pivot_longer(cols = everything(),
               names_to = "Model", names_pattern = "(.*)_", values_to = "Value")%>% 
   mutate(Measure = "R-Squared"))%>%
   mutate(Measure = str_wrap(Measure, 15))%>%
   ggplot(aes(y = Model, x = Value))+
   geom_boxplot(fill = "grey75")+
   stat_boxplot(geom = 'errorbar', color = "grey40")+
   facet_grid(~Measure)+
   theme(axis.line = element_line(color = 'grey50'),
...
Result <- ModelPerformanceCompare$values
colnames(Result) <- str_replace(colnames(Result), "~", "_")
png(paste0(here::here(), "/figures/FigureY1.png"), width = 7, height = 5, res = 600, units = "in" )
...
## Model Results: Tree Age + Plant Traits + Environmental Conditions{.tabset}
### Build Model
...
summary(gbm_regressor_bai_residuals, plotit = FALSE)%>%
  ...
  ggplot(aes(x = reorder(Label, - Importance/100),  y = Importance/100, color = Class))+
  geom_point()+
...
FSA::dunnTest(Value ~ Class, data = 
     ...
     mutate(TargetGroup = case_when(Target %in% EnvironmentalVariablesKeep ~ "Environmental Conditions",
            ...
            SourceGroup = case_when(Source %in% EnvironmentalVariablesKeep ~ "Environmental Conditions",
                           ...
                           TRUE ~ "Environmental Conditions"),
      Class = paste0(TargetGroup, ":", SourceGroup),
         Class = ifelse(Class %in% c("Plant Traits:Environment",
                                              "Environment:Plant Traits"),
                                 "Plant Traits:Environment", Class)
         ) %>%
       group_by(Class) %>%
  top_n(15, Value)) 
...
```{r include=FALSE}
InteractionsModel <- InteractionsGBMBAI %>% top_n(22,Value) %>% 
            mutate(Interactions = paste0(Target, ":", Source)) %>% select(Interactions) %>% reduce(c)
ModelTermsNuisance <- c(colnames(rgr_msh_na)[colnames(rgr_msh_na)%in%EnvironmentalVariablesKeep],
          colnames(rgr_msh_na)[colnames(rgr_msh_na)%in%PlantTraitsKeep], "Tree.Age", "julian.date.2011", 
          InteractionsModel)
ModelTerms <- c(colnames(rgr_msh_na)[colnames(rgr_msh_na)%in%EnvironmentalVariablesKeep],
          colnames(rgr_msh_na)[colnames(rgr_msh_na)%in%PlantTraitsKeep],InteractionsModel)

MainEffectsTerms <- c(colnames(rgr_msh_na)[colnames(rgr_msh_na)%in%EnvironmentalVariablesKeep],
          colnames(rgr_msh_na)[colnames(rgr_msh_na)%in%PlantTraitsKeep])

MainEffectsTermsNuisance <- c(colnames(rgr_msh_na)[colnames(rgr_msh_na)%in%EnvironmentalVariablesKeep],
          colnames(rgr_msh_na)[colnames(rgr_msh_na)%in%PlantTraitsKeep], "Tree.Age", "julian.date.2011")
TestModelFormula <- 
  paste(
  "BAI_GR", 
  "~", 
  paste(ModelTermsNuisance, collapse = " + "))
...
```{r echo=FALSE}
TestModel <- glm(TestModelFormula, 
            data = rgr_msh_na)
summary(TestModel) 
```
...
cat(paste0("McFadden's R-squared for model is ", with(summary(TestModel), 1 - deviance/null.deviance)))
...
```{r echo=FALSE}
Delete <- 
modEvA::varPart(model = TestModel, 
                groups = data.frame(Predictor = (TestModel$coefficients %>% names())[-1]) %>% 
                  mutate(Group = case_when(Predictor %in% EnvironmentalVariablesKeep ~ "Environment",
                                 Predictor %in% PlantTraitsKeep ~ "Plant Trait",
                                 Predictor %in% "Tree.Age" ~ "Interaction",
                                 Predictor %in% "julian.date.2011" ~ "Interaction",
                                 str_detect(Predictor, ":") &
                                   str_remove(str_extract(Predictor, ".+:"), ":") %in%
                                                PlantTraitsKeep &
                                   str_remove(str_extract(Predictor, ":.+"), ":") %in%
                                                PlantTraitsKeep  ~ "Plant Trait",
                                 str_detect(Predictor, ":") &
                                   str_remove(str_extract(Predictor, ".+:"), ":") %in%
                                                EnvironmentalVariablesKeep &
                                   str_remove(str_extract(Predictor, ":.+"), ":") %in%
                                                EnvironmentalVariablesKeep  ~ "Environment",
                                 str_detect(Predictor, ":") &
                                   str_remove(str_extract(Predictor, ".+:"), ":") %in%
                                                c(EnvironmentalVariablesKeep, PlantTraitsKeep) &
                                   str_remove(str_extract(Predictor, ":.+"), ":") %in%
                                                 c(EnvironmentalVariablesKeep, PlantTraitsKeep)  ~
                                   "Interaction",
                                 TRUE ~ "Interaction")))
