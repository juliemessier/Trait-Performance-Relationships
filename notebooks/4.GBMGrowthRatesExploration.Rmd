...
GBMs are an ensemble of decision trees, nut they are fit sequentially. We call GBMs an ensemble of weak learners as each subsequent tree is an attempt to correct the errors of the previous tree. Thus, while one tree, by itself, can not describe the relationships, with the use of all the trees, we can. Below is a figure by Bradly Bohemke that attempts to illustrate how each subsequent tree improvs the fit on the data.

## Changes made to raw data

Below is a list of the changes made to the raw data:

* Environental varibles - these are the princple compoeets (1-5) from a PCA that was fit on all the enevirobental vairbles. We visialsed the PCA and used the ageinvectors to help figure which envonntal conditn best explained that PC. There were 5 - `Soil Fertility, Light, Temperature,  pH, Soil.Humidity.Depth, and Slope`. 
* Remove the influence of sampling date - some of the vairbles vary with when they were measured. Assuing a linear relationship between sample date and the vairbles, we extracted the residieal from a `lm` model with Variable ~ Sample date when the pvalue<0.05. This realuted in changes for the following vairbles - `Estem, Branching Distance, Stem Wood Density,  LMA, LNC, d15N, Ks, X.Sapwood, d13C, Soil.Fertility, Light, Temperature, pH`  
...
Assessing how, when we hold everything else constant, what the realtionships are betweern growth rate and the predictor.
...
How does the model perform when we use the true indiivual trait value?
...
Let's explore the interactcions in these data.
...
```{r include=FALSE}
PDPInteractions <-
  map2(.x = InteractionsGBMBAI %>% filter(Value>1.01763794414045e-14) %>% select(Source) %>% reduce(c), 
      .y = InteractionsGBMBAI %>% filter(Value>1.01763794414045e-14) %>% select(Target) %>% reduce(c),
      ~{
         LabelX <- labels_rgr_msh %>% filter(Label!= "Soil pH") %>% filter(Feature %in%.x) %>% 
           select(Label) %>% reduce(c)
         LabelY <- labels_rgr_msh %>% filter(Label!= "Soil pH") %>% filter(Feature %in%.y) %>% 
...
write_csv(rgr_msh_na %>% filter(Group == "Train")%>%
          ...
          here("data/RGRH20NoDateBIOSpecies.csv"))

write_csv(rgr_msh_na %>% filter(Group == "Train")%>%
            select(any_of(c(EnvironmentalVariablesKeep, "Species", "Tree.Age", "BAI_GR"))),
          here("data/RGRH20NoDateBAISpecies.csv"))
```
...
```{r include=FALSE}
PDPInteractions <-
  map2(.x = InteractionsGBMBIOSpecies %>% filter(Value>0.15) %>% select(Source) %>% reduce(c), 
       .y = InteractionsGBMBIOSpecies %>% filter(Value>0.15) %>% select(Target) %>% reduce(c), ~{
...
```{r include=FALSE}
PDPInteractions <-
  map2(.x = InteractionsGBMBAISpecies %>% filter(Value>0.15) %>% select(Source) %>% reduce(c), 
      .y = InteractionsGBMBAISpecies %>% filter(Value>0.15) %>% select(Target) %>% reduce(c),
      ~{
         LabelX <- labels_rgr_msh %>% filter(Label!= "Soil pH") %>% filter(Feature %in%.x) %>% 
           select(Label) %>% reduce(c)
         LabelY <- labels_rgr_msh %>% filter(Label!= "Soil pH") %>% filter(Feature %in%.y) %>% 
